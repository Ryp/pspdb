...
pub fn decode(allocator: std.mem.Allocator, bytes: []const u8) ![]u8 {
    ...
    if (try expandPayload(allocator, output[0..expected], if (attributes & 1 != 0) elf_size else null)) |expanded| {
...
fn expandPayload(allocator: std.mem.Allocator, payload: []const u8, declared_size: ?usize) !?[]u8 {
    ...
    if (declared_size) |size| {
        ...
        const actual = (if (gzip)
            @import("containers.zig").decodeGzipMemberInto(payload, output)
        else if (kl)
            @import("kle.zig").decodeInto(payload, output)
        else
            @import("lzr.zig").decodeInto(payload, output)) catch |err| return switch (err) {
    ...
    return if (gzip)
        try @import("containers.zig").decodeGzipMemberBounded(allocator, payload, max_decoded_size)
    else if (kl)
        try @import("kle.zig").decode(allocator, payload)
    else
        try @import("lzr.zig").decode(allocator, payload);
}
...
test "contextual KL enforces declared output only when the envelope declares compression" {
    ...
    for ([_][]const u8{ "KL3E", "KL4E" }) |magic| {
        ...
        const exact = (try expandPayload(fixed.allocator(), &payload, 3)).?;
        ...
        try std.testing.expectError(error.PrxSizeMismatch, expandPayload(allocator, &payload, 2));
        try std.testing.expectError(error.PrxSizeMismatch, expandPayload(allocator, &payload, 4));
        try std.testing.expectError(error.InvalidKle, expandPayload(allocator, payload[0 .. payload.len - 1], 3));
        const resource = (try expandPayload(allocator, &payload, null)).?;
    ...
    const expanded = (try expandPayload(allocator, coded, 8)).?;
    defer allocator.free(expanded);
    try std.testing.expectEqualStrings("offsets!", expanded);
    try std.testing.expectError(error.PrxSizeMismatch, expandPayload(allocator, coded, 7));
    try std.testing.expectError(error.PrxSizeMismatch, expandPayload(allocator, coded, 9));
}
...
test "contextual LZR expands resource streams and enforces compressed envelope size" {
    ...
    var storage: [3]u8 = undefined;
    var fixed = std.heap.FixedBufferAllocator.init(&storage);
    const exact = (try expandPayload(fixed.allocator(), direct, 3)).?;
    defer fixed.allocator().free(exact);
    try std.testing.expectEqualStrings("abc", exact);
    try std.testing.expectError(error.PrxSizeMismatch, expandPayload(allocator, direct, 2));
    try std.testing.expectError(error.PrxSizeMismatch, expandPayload(allocator, direct, 4));
    try std.testing.expectError(error.InvalidLzr, expandPayload(allocator, direct[0 .. direct.len - 1], 3));

    const coded = "2RLZ\x08\xc8\x63\x3d\xbe\x16\x38\x4a\xd2\x32\xf6\x60\x00\x00\x00";
    const resource = (try expandPayload(allocator, coded, null)).?;
    defer allocator.free(resource);
    ...
    try std.testing.expectError(error.PrxSizeMismatch, expandPayload(allocator, coded, 7));
    try std.testing.expectError(error.PrxSizeMismatch, expandPayload(allocator, coded, 9));
    try std.testing.expectError(error.InvalidLzr, expandPayload(allocator, coded[0 .. coded.len - 1], 8));
    try std.testing.expectError(error.InvalidPrxSize, expandPayload(allocator, coded, max_decoded_size + 1));
    try std.testing.expectError(error.InvalidPrxSize, expandPayload(allocator, coded, 0));
}
...
test "contextual gzip checks declared length after a complete member" {
    ...
    const allocator = std.testing.allocator;
    const output = (try expandPayload(allocator, member ++ "\x00envelope tail", 3)).?;
    defer allocator.free(output);
    try std.testing.expectEqualStrings("abc", output);
    try std.testing.expectError(error.PrxSizeMismatch, expandPayload(allocator, member, 2));
    try std.testing.expectError(error.PrxSizeMismatch, expandPayload(allocator, member, 4));
    try std.testing.expectError(error.InvalidPrxSize, expandPayload(allocator, member, max_decoded_size + 1));
    try std.testing.expectError(error.InvalidPrxSize, expandPayload(allocator, member, 0));
}

fn signedFixture(allocator: std.mem.Allocator, payload: []const u8) ![]u8 {
    var input: std.Io.Reader = .fixed(payload);
    var output: std.Io.Writer.Allocating = .init(allocator);
    defer output.deinit();
    try @import("zig_psp_prx_encrypt").encrypt(allocator, &input, payload.len, &output.writer);
...
test "PRX preserves unaligned input and rejects truncation or modified headers" {
    ...
    const fixture = try signedFixture(allocator, payload);
...
test "concurrent PRX jobs retain independent KIRK state and owned results" {
    const allocator = std.testing.allocator;
    const first = try signedFixture(allocator, "\x7fELF first concurrent PRX");
    defer allocator.free(first);
    const second = try signedFixture(allocator, "\x7fELF second concurrent PRX");
