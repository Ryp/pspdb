...
const zig_psp_pbp = @import("zig_psp_pbp");

/// These readers borrow source slices; callers own names, hashes and storage.
pub fn walk_sce(bytes: []const u8, context: anytype, comptime emit: anytype) !void {
...
pub fn walk_vmp(bytes: []const u8, context: anytype, comptime emit: anytype) !void {
...
test "VMP preserves the complete card and rejects inconsistent wrapper bounds" {
    ...
    try walk_vmp(bytes[0..0x20080], bytes[0x80..0x20080], Consumer.emit);
    try std.testing.expectError(error.InvalidVmp, walk_vmp(bytes[0..0x2007f], &.{}, Consumer.emit));
    try std.testing.expectError(error.InvalidVmp, walk_vmp(&bytes, &.{}, Consumer.emit));
    std.mem.writeInt(u32, bytes[4..8], 0x7f, .little);
    try std.testing.expectError(error.InvalidVmp, walk_vmp(bytes[0..0x20080], &.{}, Consumer.emit));
}

pub fn parse_pbp(bytes: []const u8) !zig_psp_pbp.Pbp {
...
pub fn embedded_psp(bytes: []const u8, start: usize) ?struct { offset: usize, size: usize } {
...
pub fn walk_elf(bytes: []const u8, context: anytype, comptime emit: anytype) !void {
    var cursor: usize = 0;
    while (embedded_psp(bytes, cursor)) |item| {
...
test "Zig-PSP PBP reader borrows slices and preserves the entire final section" {
    ...
    file.close(io);
    try zig_psp_pbp.unpack_pbp(allocator, io, input_path, output_path);
    ...
    _ = try parse_pbp(&bytes); // Observed in the retail echochrome demo.
    std.mem.writeInt(u32, bytes[4..8], 0x20001, .little);
    try std.testing.expectError(error.InvalidPbp, parse_pbp(&bytes));
    try std.testing.expectError(error.InvalidPbp, parse_pbp(bytes[0..39]));
}
