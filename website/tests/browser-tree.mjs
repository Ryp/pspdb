...
const result=await evaluate(`(async()=>{
 ...
 const descendantsOf=node=>{
  const result=[],pending=[...node.children];
  while(pending.length){const child=pending.pop();result.push(child);for(const next of child.children)pending.push(next);}
  return result;
 };
 ...
 for(const item of [...nodes.values()].filter(n=>n.displayName&&n.extraction)){
  ...
  const descendants=descendantsOf(item);
  ...
  for(const nested of descendants.filter(n=>n.extraction)){
   check(collapsed.has(nested.index),'Opening parent keeps nested extracted files collapsed');
  ...
  if(nestedFolder){
   ...
   check(collapsed.has(nestedFolder.index)&&nestedFolder.children.every(n=>!visible.includes(n)),'Reopening preserves nested collapse choices');
  }
 }
 ...
 jump(sizeNode);
 check(rowElements.get(sizeNode.index).querySelector('.size').textContent===formatSize(sizeNode.size),'Rendered readable sizes');
 check(rowElements.get(sizeNode.index).querySelector('.size').title===number.format(sizeNode.size)+' bytes','Exact byte tooltip');
 ...
 jump(nested);
 rowElements.get(nested.index).querySelector('.toggle').click();
 check(collapsed.has(nested.index),'Nested folder should collapse');
 check(!visible.includes(leaf),'Nested contents should hide');
 jump(branch); rowElements.get(branch.index).querySelector('.toggle').click();
 check(!visible.includes(nested),'Parent should hide nested folder');
 rowElements.get(branch.index).querySelector('.toggle').click();
 ...
 jump(nested);
 check(rowElements.get(nested.index).getAttribute('aria-expanded')==='false','Disclosure aria state');
 select(nested);
 document.dispatchEvent(new KeyboardEvent('keydown',{key:'j',bubbles:true}));
 check(!rowElements.get(selected.index).hidden && selected!==leaf,'j must skip hidden descendants');
 for(const n of nodes.values())if(expandable(n))collapsed.add(n.index);render();select(visible[0]);
 check(visible.length===1 && selected===root,'Collapse all retains the aggregate root');
 ...
 check(root.path===''&&url(root)==='#','Aggregate root preserves category URL paths');
 check(root.children.every(n=>n.depth===1),'Categories are children of the aggregate root');
 ...
 const query=async value=>{search.value=value;search.dispatchEvent(new Event('input',{bubbles:true}));await searchSettled;};
 ...
 check(visible.every(n=>n.type==='file'&&n.displayPath.toLowerCase().includes('eboot.bin')),'Flat results contain matching files without ancestor rows');
 const resultNode=visible[0], resultRow=rowElements.get(resultNode.index);
 const resultAncestors=[];for(let node=resultNode.parent;node;node=node.parent)resultAncestors.push(node);
 check(resultNode.nameElement.textContent===resultNode.displayPath&&resultAncestors.some(n=>n.displayName&&resultNode.displayPath.includes(label(n))),'One-line path uses readable ISO label');
 ...
 const sourceTerms=label(resultAncestors.find(n=>n.displayName)).toLowerCase().split(/\\s+/).filter(term=>!/^[0-9a-f]{8,64}$/.test(term));
 const sourceTitle=sourceTerms.join(' ');
 await query(sourceTitle.toUpperCase()+' EBOOT');
 check(visible.some(n=>n.type==='file'),'Combined title and filename query');
 check(visible.filter(n=>n.type==='file').every(n=>sourceTerms.every(term=>n.displayPath.toLowerCase().includes(term))&&n.displayPath.toLowerCase().includes('eboot')),'AND matching across ancestor title and filename');
 const file=visible.find(n=>n.type==='file'); select(file);
 document.dispatchEvent(new KeyboardEvent('keydown',{key:'l',bubbles:true}));
 check(selected===file,'Flat results do not navigate to hidden tree nodes');
 await query(sourceTitle.toUpperCase());
 ...
 for(const key of ['size','hash','name']) {
  ...
  button.click(); await searchSettled;
 ...
 const exactOccurrences=nodes.filter(n=>n.type==='file').filter(n=>n.hash===container.hash);
 await query(container.hash);
 ...
 await query(container.hash.slice(0,12).toUpperCase());
 ...
 await query(container.hash);
 await query(container.name);
 check(descendantsOf(container).filter(n=>n.type==='file').every(n=>visible.includes(n)),'Changing hash query to filename restores ancestor-text matching');
 ...
 await query('<img src=x onerror=alert(1)>');
 ...
 for(const target of [file,[...nodes.values()].find(n=>n.extraction&&n.parent!==root)]) {
  for(const n of nodes.values())if(expandable(n))collapsed.add(n.index);
  render();select(visible[0]);
  const beforeJump=new Set(collapsed);
  await query(target.hash);
  document.getElementById('sort-size').click(); await searchSettled;
  select(target);
  ...
  for(let parent=target.parent;parent;parent=parent.parent)ancestors.add(parent.index);
  ...
  const rect=rowElements.get(target.index).getBoundingClientRect();
  ...
  check(rowElements.get(target.index).getAttribute('aria-selected')==='true','Double-click announces selection');
 }
 collapsed.clear();render();select(selected || visible[0]);
 document.getElementById('tree').focus();
 document.dispatchEvent(new KeyboardEvent('keydown',{key:'End',bubbles:true}));
 check(selected===visible.at(-1)&&rowElements.get(selected.index).isConnected,'End mounts last row');
 const lastRect=rowElements.get(selected.index).getBoundingClientRect();
 ...
 check(selected===visible[0]&&rowElements.get(selected.index).isConnected,'Home mounts first row');
 const topRect=rowElements.get(selected.index).getBoundingClientRect();
 check(topRect.top>=document.querySelector('thead').getBoundingClientRect().bottom-1,'First row below sticky header');
 return {rows:initial,result:'PASS: nested fold state, hidden-row keyboard skipping, root boundary, jump reveal, expand/collapse all, aria state, async search, title/path/hash matching, filtered navigation, empty state, search shortcuts and restoration'};
})()`);
...
const download=await evaluate(`(async()=>{
 ...
 if(rowElements.get(file.index).children.length!==4)throw new Error('Download column missing');
...
const extractedDownload=await evaluate(`(async()=>{
 ...
 const row=rowElements.get(source.index);
 ...
 if(collapsed.has(source.index)||!visible.includes(source.children[0]))throw new Error('Cannot expand extracted file');
 document.dispatchEvent(new KeyboardEvent('keydown',{key:'ArrowLeft',bubbles:true}));
 if(!collapsed.has(source.index)||visible.includes(source.children[0]))throw new Error('Cannot collapse extracted file');
 const child=[...nodes.values()].find(n=>n.path.startsWith(source.path+'/')&&n.type==='file'&&!n.extraction&&n.size>0);
 document.getElementById('tree-search').value=child.hash;await applySearch();
...
const extractionErrors=await evaluate(`(async()=>{
 ...
 for(const node of nodes.values())if(node.extraction)collapsed.add(node.index);
 render();
 ...
 for(const node of [source,edat,inline]){
  jump(node);
  const row=rowElements.get(node.index), diagnostic=row.querySelector('.extraction-error');
 ...
 check(!rowElements.get(sibling.index).querySelector('.extraction-error'),'Inline failure does not leak to same-hash sibling');
 ...
 check(!rowElements.get(edat.index)?.isConnected,'Distant failed source is virtualized');
 document.getElementById('tree-search').value='UnsupportedEdat';await applySearch();
 check(visible.length===1&&visible[0]===edat,'Error search finds an empty failed extraction');
 check(rowElements.get(edat.index).querySelector('.error-message mark')?.textContent==='UnsupportedEdat','Error matches highlight after remount');
 check(!document.getElementById('selected-error').hidden,'Search selection retains full error details');
 const input=document.getElementById('tree-search');
 input.value='sibling';const superseded=applySearch();
 input.value='UnsupportedEdat';await applySearch();await superseded;
 check(visible.length===1&&visible[0]===edat,'Older worker results never replace newer input');
 input.value='sibling';const cancelled=applySearch();clearSearch();await cancelled;
 // Wait for a later worker reply to prove pending work cannot overwrite clear.
 const worker=searchWorker, handler=worker.onmessage;
 await new Promise(resolve=>{
  worker.onmessage=event=>{
   if(event.data.type==='results'&&event.data.version===searchVersion){worker.onmessage=handler;resolve();}
   else handler(event);
  };
  worker.postMessage({type:'search',version:searchVersion,terms:['sibling'],sort:null,direction:1});
 });
